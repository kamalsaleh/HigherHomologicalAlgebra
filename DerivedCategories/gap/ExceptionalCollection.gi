...
InstallMethod( CreateStrongExceptionalCollection,
  ...
  function( full )
    ...
    vertices_labels := List( [ 1 .. Length( SetOfKnownObjects( full ) ) ],
...
InstallMethod( BasisOfPathsOp,
  ...
  function( collection, indices )
    ...
    while Length( B ) < dim - Length( arrows ) do
      ...
      long_paths := long_paths{ [ p + 1 .. Length( long_paths ) ] };
      
      long_paths_labels := long_paths_labels{ [ p + 1 .. Length( long_paths_labels ) ] };
...
InstallMethod( TriangulatedSubcategory,
  ...
  function( collection )
    local name, homotopy_cat, full, D, C;
    
    name := Concatenation(
              "The triangulated subcategory generated by {",
              JoinStringsWithSeparator( collection!.vertices_labels, ", " ),
              "}"
            );
    
    homotopy_cat := AmbientCategory( collection );
    
    full := FullSubcategory( homotopy_cat, name
                             : FinalizeCategory := false
                           );
    
    D := DefiningCategory( homotopy_cat );
    
    if IsAdditiveClosureCategory( D ) then
      
      C := UnderlyingCategory( D );
      
      if IsAlgebroid( C ) and IsAcyclicQuiver( UnderlyingQuiver( C ) ) then
          
          AddIsWellDefinedForObjects( full,
            function( full, o )
              local homotopy_cat, I, r;
              
              homotopy_cat := AmbientCategory( full );
              
              I := EquivalenceOntoDerivedCategory( homotopy_cat );
              
              o := UnderlyingCell( o );
              
              if not IsWellDefined( o ) then
                return false;
              fi;
              
              r := RemainderObject( o, collection );
              
              # checking whether an object is zero is faster in derived category than in homotopy category
              return IsZeroForObjects( I( r ) );
      ...
      if IsAcyclicQuiver( UnderlyingQuiver( D ) ) then
...
InstallMethod( FullSubcategoryGeneratedByIndecProjectiveObjects,
  ...
  function( full_subcategory_by_projs )
    ...
    name := Concatenation( r[ 1 ], "Full subcategory generated by the ", String( Length( projs ) ),
...
InstallMethod( FullSubcategoryGeneratedByIndecInjectiveObjects,
  ...
  function( full_subcategory_by_injs )
    ...
    name := Concatenation( r[ 1 ], "Full subcategory generated by the ", String( Length( injs ) ),
...
InstallGlobalFunction( RandomQuiverAlgebraWhoseIndecProjectiveRepsAreStrongExceptionalCollection,
  function( field, nr_vertices, nr_arrows, nr_relations )
    ...
    if not IsEmpty( rel ) then
      ...
      name := Concatenation( "(", Name( A ), ") / [ ", String( Length( rel ) ), " relations ]" );
      name_op := Concatenation( "(", Name( OppositeAlgebra( A ) ), ") / [ ", String( Length( rel ) ), " relations ]" );
